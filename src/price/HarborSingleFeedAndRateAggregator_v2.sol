...
/// @title Harbor Single Feed and Rate Aggregator V2
...
contract HarborSingleFeedAndRateAggregator_v2 is
    IWrappedPriceOracle,
    UUPSUpgradeable,
    ReentrancyGuardTransientUpgradeable,
    BaoOwnable
{
    ...
    /// @notice Whether to invert the feed price (default: false)
    /// @dev When true, calculates 1e36 / feedPrice instead of feedPrice / priceDivisor
    ///      Useful for USD-pegged tokens where feed is X/USD but we need USD/X
    ...
    /// @param invertPrice_ Whether to invert the feed price (default: false)
    function initialize(
        address owner_,
    ...
    function initializeFeeds(uint64 firstFeedMaxAge, uint256 firstFeedMaxDev) external onlyOwner {
    ...
    /// @dev V2: Price does NOT multiply by rate - uses feed price directly
    ///      If invertPrice is true, calculates 1e36 / feedPrice (useful for USD-pegged tokens)
    /// @return price The current price, always 18 decimals
    function _getPrice() internal view returns (uint256 price) {
        ...
        uint256 finalPrice;
        if (invertPrice) {
            // Invert the price: calculate 1e36 / (firstFeedPrice / priceDivisor)
            // Feed gives X/USD (e.g., ETH/USD = 2964 means 2964 USD per ETH)
            // We need USD/X (e.g., USD/ETH = 1/2964 means 0.000337 ETH per USD)
            // For feeds with divisors (like MCAP), apply divisor first, then invert
            // Formula: (1e18 * 1e18 * priceDivisor) / firstFeedPrice = 1e36 * priceDivisor / firstFeedPrice
            finalPrice = Math.mulDiv(1e18 * priceDivisor, 1e18, firstFeedPrice);
        } else {
            // V2: Direct feed price without rate multiplication
            // Apply divisor to final price for normalization
            finalPrice = firstFeedPrice / priceDivisor;
        }
    ...
    /// @notice Set whether to invert the feed price
    /// @dev When true, calculates 1e36 / feedPrice instead of feedPrice / priceDivisor
    ///      Useful for USD-pegged tokens where feed is X/USD but we need USD/X
