...
/// @notice stETH/MAG7.i26 oracle (rate: Chainlink wstETH/stETH feed, price: (1 stETH in USD) / ((sum of 7 stocks) / index price)).
...
contract Oracle_stETH_MAG7i26 is HarborPriceAggregator_v3 {
    ...
    function latestAnswer() external view override(IWrappedPriceOracle) returns (uint256, uint256, uint256, uint256) {
        ...
        // Price = (1 stETH in USD) / indexedValue = stETH per MAG7.i26
        // Since we want 1 stETH offset, we use baseUsdPrice directly
        uint256 price = Math.mulDiv(baseUsdPrice, 1e18, indexedValue);
