...
/// @title Harbor Double Feed and Rate Aggregator V2
...
contract HarborDoubleFeedAndRateAggregator_v2 is
    IWrappedPriceOracle,
    UUPSUpgradeable,
    ReentrancyGuardTransientUpgradeable,
    BaoOwnable
{
    ...
    /// @dev When true, converts from secondFeed to firstFeed instead of firstFeed to secondFeed
    ...
    /// @dev V2: Price does NOT multiply by rate - converts directly from first feed to second feed
    ///      If invertPrice is true, converts from second feed to first feed instead
    /// @return price The current price, always 18 decimals
    function _getPrice() internal view returns (uint256 price) {
        ...
        PriceOracle_v1.Feed memory secondFeedData =
            PriceOracle_v1.Feed({priceFeed: AggregatorV3Interface(secondFeed), decimals: secondFeedDecimals});
        ...
        if (invertPrice) {
            ...
            // Formula: (secondFeedPrice * 1e18) / (firstFeedPrice * priceDivisor)
            finalPrice = Math.mulDiv(secondFeedPrice, 1e18, Math.mulDiv(firstFeedPrice, priceDivisor, 1));
        } else {
            // V2: Direct conversion from first feed to second feed without rate multiplication
            // Convert first price to second feed currency with divisor normalization
            finalPrice = Math.mulDiv(Math.mulDiv(firstFeedPrice, priceDivisor, 1), 1e18, secondFeedPrice);
        }
