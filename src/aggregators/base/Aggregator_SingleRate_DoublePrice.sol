...
/// @notice Oracle with rate from one CL feed and price = rate * (firstFeed/secondFeed) (e.g. shMON/BTC).
/// @custom:oz-upgrades-unsafe-allow state-variable-immutable constructor
// solhint-disable-next-line contract-name-capwords
abstract contract Aggregator_SingleRate_DoublePrice is HarborAggregator_v3 {
    ...
    function latestAnswer() external view override(IWrappedPriceOracle) returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 pairPrice = DoubleFeedPriceLib.getPrice(
            FIRST_FEED,
            FIRST_FEED_DECIMALS,
        ...
        uint256 price = Math.mulDiv(rate, pairPrice, 1e18);
