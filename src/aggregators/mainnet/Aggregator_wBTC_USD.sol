...
contract Aggregator_wBTC_USD is HarborAggregator_v3 {
    ...
    function latestAnswer() external view override(IWrappedPriceOracle) returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 wbtcBtc = ChainlinkFeedLib.latestAnswerNormalized(
            FIRST_FEED,
            FIRST_FEED_DECIMALS,
            FIRST_FEED_HEARTBEAT
        );
        uint256 btcUsd = ChainlinkFeedLib.latestAnswerNormalized(
            SECOND_FEED,
            SECOND_FEED_DECIMALS,
            SECOND_FEED_HEARTBEAT
        );

        // Both feed values are normalized to 1e18, so the product is 1e36.
        // Divide once by 1e18 to bring price back to 1e18 precision, then apply divisor.
        uint256 normalizedPrice = Math.mulDiv(wbtcBtc, btcUsd, 1e18);
        uint256 price = normalizedPrice / PRICE_DIVISOR;
