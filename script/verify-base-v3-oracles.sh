...
# Re-verify all Base v3 oracles
# This script reads from deployments/base/v3-oracles.json and verifies all contracts
...
if [[ ! -f "$DEPLOYMENT_FILE" ]]; then
  ...
  echo "   Run the deployment script first: ./script/deploy-base-v3-oracles.sh"
  exit 1
fi
...
verify_contract() {
  ...
  local code=$("$CAST" code "$address" --rpc-url "$BASE_RPC_URL" 2>/dev/null | head -1 || echo "0x")
  if [[ "$code" == "0x" ]]; then
