...
# Re-verify all Mainnet v3 oracles
# This script reads from deployments/mainnet/v3-oracles.json and verifies all contracts
...
# Deployment file
DEPLOYMENT_FILE="deployments/mainnet/v3-oracles.json"

if [[ ! -f "$DEPLOYMENT_FILE" ]]; then
  echo "❌ ERROR: Deployment file not found: $DEPLOYMENT_FILE"
  echo "   Run the deployment script first: ./script/mainnet/deploy/deploy-mainnet-v3-oracles.sh"
  exit 1
fi
...
verify_contract() {
  ...
  local code=$("$CAST" code "$address" --rpc-url "$MAINNET_RPC_URL" 2>/dev/null | head -1 || echo "0x")
  if [[ "$code" == "0x" ]]; then
