...
import {Aggregator_hsstETH_GOLD_USD_mainnet} from "@harbor-price/mainnet/Aggregator_hsstETH_GOLD_USD_mainnet.sol";

/// @notice hsstETH-GOLD/USD on mainnet: what it announces, what it reads, what it composes,
///         and how it fails when a source misbehaves
contract Aggregator_hsstETH_GOLD_USD_mainnet_Test is WiredLeveragedTokenHarness {
    ...
    function _rateSource() internal pure override returns (OracleSource memory) {
        return OracleSource({at: MainnetRateSources.MINTER_HSSTETH_GOLD, kind: SourceKind.MinterLeveragedPrice});
    }

    function _priceFeeds() internal pure override returns (address[] memory feeds) {
        feeds = new address[](1);
        feeds[0] = XAU_USD.FEED;
    }

    function _createAggregator() internal override returns (address) {
        return address(new Aggregator_hsstETH_GOLD_USD_mainnet());
    }
}
