...
import {Aggregator_USDE_EUR_mainnet} from "@harbor-price/mainnet/Aggregator_USDE_EUR_mainnet.sol";

/// @notice USDE/EUR on mainnet: what it announces, what it reads, what it composes,
///         and how it fails when a source misbehaves
contract Aggregator_USDE_EUR_mainnet_Test is WiredDoubleFeedHarness {
    ...
    function _priceFeeds() internal pure override returns (address[] memory feeds) {
        ...
        feeds[1] = EUR_USD.FEED;
    }

    function _createAggregator() internal override returns (address) {
        return address(new Aggregator_USDE_EUR_mainnet());
    }
}
