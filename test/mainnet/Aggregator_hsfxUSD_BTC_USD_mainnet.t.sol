...
import {Aggregator_hsfxUSD_BTC_USD_mainnet} from "@harbor-price/mainnet/Aggregator_hsfxUSD_BTC_USD_mainnet.sol";

/// @notice hsfxUSD-BTC/USD on mainnet: what it announces, what it reads, what it composes,
///         and how it fails when a source misbehaves
contract Aggregator_hsfxUSD_BTC_USD_mainnet_Test is WiredLeveragedTokenHarness {
    ...
    function _rateSource() internal pure override returns (OracleSource memory) {
        return OracleSource({at: MainnetRateSources.MINTER_HSFXUSD_BTC, kind: SourceKind.MinterLeveragedPrice});
    }
    ...
    function _createAggregator() internal override returns (address) {
        return address(new Aggregator_hsfxUSD_BTC_USD_mainnet());
    }
}
