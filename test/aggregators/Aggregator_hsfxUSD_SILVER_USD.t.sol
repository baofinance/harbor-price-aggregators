...
contract Aggregator_hsfxUSD_SILVER_USD_Test is LeveragedTokenUSDAggregatorTestBase {
    function _contractName() internal pure override returns (string memory) {
        return type(Aggregator_hsfxUSD_SILVER_USD).name;
    }

    function _expectedBaseName() internal pure override returns (string memory) {
        return "hsfxUSD-SILVER";
    }
    ...
    function _createAggregator(
        ...
        return
            IHarborPriceAggregatorV3(
                address(new Aggregator_hsfxUSD_SILVER_USD(minter, underlyingUsdFeed, underlyingUsdHeartbeat, 1, false))
            );
    }

    function _createWithZeroMinter() internal override {
        new Aggregator_hsfxUSD_SILVER_USD(address(0), address(mockUnderlyingUsdFeed), DEFAULT_HEARTBEAT, 1, false);
    }

    function _createWithZeroUnderlying() internal override {
        new Aggregator_hsfxUSD_SILVER_USD(address(mockMinter), address(0), DEFAULT_HEARTBEAT, 1, false);
    }
    ...
    function _createWithZeroDivisor() internal override {
        new Aggregator_hsfxUSD_SILVER_USD(
            address(mockMinter),
