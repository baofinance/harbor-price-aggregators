...
abstract contract SingleFeedNoRateAggregatorTestBase is OracleSourceConformance {
    ...
    /// @notice Returns the contract name (e.g., "Aggregator_PAXG_USD")
    ...
    function _sources() internal view override returns (OracleSource[] memory sources) {
        sources = new OracleSource[](1);
        sources[0] = OracleSource({at: address(mockPriceFeed), kind: SourceKind.ChainlinkFeed});
    }
    ...
    function setUp() public virtual {
        ...
        mockPriceFeed.setAnswer(2000e8, block.timestamp); // Default price

        aggregator = _createAggregator(address(mockPriceFeed), DEFAULT_HEARTBEAT, 1, false);
    }
    ...
    function test_rateProvider() public view {
        assertEq(aggregator.rateProvider(), address(0));
    }
    ...
    function test_latestAnswer_staleFeed_reverts() public {
        ...
        mockPriceFeed.setAnswer(2000e8, staleTime);
