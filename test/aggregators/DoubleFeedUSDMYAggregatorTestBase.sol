...
import {MockUSDMY} from "@harbor-test/mock/MockUSDMY.sol";
...
/// @title Base test contract for double-feed v3 aggregators with USDMY rate (USDMY pattern)
/// @notice Provides all tests; concrete contracts only implement factory + identity
abstract contract DoubleFeedUSDMYAggregatorTestBase is Test {
    MockUSDMY mockUSDMY;
    ...
    uint256 constant VALID_USDMY_RATE = 1.05e18;
    ...
    /// @notice Returns the contract name (e.g., "Aggregator_USDMY_ETH")
    ...
    /// @notice Deploy with zero usdmy (for revert test)
    function _createWithZeroUSDMY() internal virtual;
    ...
    function setUp() public virtual {
        ...
        mockUSDMY = new MockUSDMY();
        mockFirstFeed = new MockAggregatorV3(8);
        mockSecondFeed = new MockAggregatorV3(8);

        mockUSDMY.setAssetsPerShare(VALID_USDMY_RATE);
        mockFirstFeed.setAnswer(1e8, block.timestamp); // USDM/USD = 1.0
    ...
    function test_constructor_revertsOnZeroUSDMY() public {
        vm.expectRevert();
        _createWithZeroUSDMY();
    }
    ...
    function test_rateProvider() public view {
        assertEq(aggregator.rateProvider(), address(mockUSDMY));
    }
    ...
    function test_latestAnswer_returnsValidTuple() public view {
        ...
        assertEq(r1, VALID_USDMY_RATE, "rate1");
    ...
    function test_latestAnswer_rateBelowMin_reverts() public {
        ...
        mockUSDMY.setAssetsPerShare(lowRate);

        vm.expectRevert(abi.encodeWithSelector(USDMYRateLib.InvalidRate.selector, lowRate));
        aggregator.latestAnswer();
    }
