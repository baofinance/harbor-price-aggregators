...
/// @notice stETH/GOOGL on arbitrum: what it announces, what it reads, what it composes,
///         and how it fails when a source misbehaves
contract Aggregator_stETH_GOOGL_arbitrum_Test is WiredDoubleFeedHarness {
    ...
    function _priceFeeds() internal pure override returns (address[] memory feeds) {
        ...
        feeds[1] = GOOGL_USD.FEED;
    }

    function _createAggregator() internal override returns (address) {
        return address(new Aggregator_stETH_GOOGL_arbitrum());
    }
}
