...
/// @title Base test contract for Base multi-feed normalized v3 aggregators (BOM5 pattern: 5 feeds, rate feed, base USD feed, normalization factors)
/// @notice Provides all tests; concrete contracts only implement factory + identity
abstract contract BaseMultiFeedNormalizedAggregatorTestBase is WrappedPriceOracleConformance {
    MockAggregatorV3 mockRateFeed;
    MockAggregatorV3 mockBaseUsdFeed;
    MockAggregatorV3[5] mockFeeds;

    uint256 constant DEFAULT_HEARTBEAT = 86400; // 24 hours
    uint256 constant VALID_RATE = 1.15e18; // 1.15 wstETH/stETH
    uint256 constant FEED_COUNT = 5;
    // Normalization factors (18 decimals) - example values for testing
    uint256 constant NORM_FACTOR_0 = 168_200_000_000_000_000_000; // 168.2e18
    uint256 constant NORM_FACTOR_1 = 589_500_000_000_000_000_000_000; // 589500e18
    uint256 constant NORM_FACTOR_2 = 421_000_000_000_000_000_000_000; // 421000e18
    uint256 constant NORM_FACTOR_3 = 998_840_000_000_000_000; // ~0.99884e18
    uint256 constant NORM_FACTOR_4 = 1_000_000_000_000_000_000; // 1e18
    ...
    /// @notice Returns the contract name (e.g., "Aggregator_stETH_BOM5")
    ...
    /// @notice Deploy with zero normalization factor at index (for revert test)
    function _createWithZeroNormFactor(uint256 normIndex) internal virtual;
    ...
    function setUp() public virtual {
        ...
        // Create 5 mock feeds (all with 8 decimals like meme coin feeds)
        for (uint256 i = 0; i < FEED_COUNT; i++) {
            mockFeeds[i] = new MockAggregatorV3(8);
            // Set prices: 0.1e8, 0.2e8, 0.3e8, etc. (meme coin prices)
            mockFeeds[i].setAnswer(int256((1 + i) * 1e7), block.timestamp);
        }
    ...
    function test_constructor_revertsOnZeroNormFactor0() public {
        vm.expectRevert();
        _createWithZeroNormFactor(0);
    }

    function test_constructor_revertsOnZeroNormFactor1() public {
        vm.expectRevert();
        _createWithZeroNormFactor(1);
    }

    function test_constructor_revertsOnZeroNormFactor2() public {
        vm.expectRevert();
        _createWithZeroNormFactor(2);
    }

    function test_constructor_revertsOnZeroNormFactor3() public {
        vm.expectRevert();
        _createWithZeroNormFactor(3);
    }

    function test_constructor_revertsOnZeroNormFactor4() public {
        vm.expectRevert();
        _createWithZeroNormFactor(4);
    }
    ...
    function test_latestAnswer_staleFeed0_reverts() public {
        uint256 staleTime = block.timestamp - DEFAULT_HEARTBEAT - 43;
        mockFeeds[0].setAnswer(1e7, staleTime);
