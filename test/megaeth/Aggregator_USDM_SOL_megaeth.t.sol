...
import {Aggregator_USDM_SOL_megaeth} from "@harbor-price/megaeth/Aggregator_USDM_SOL_megaeth.sol";

/// @notice USDM/SOL on megaeth: what it announces, what it reads, what it composes,
///         and how it fails when a source misbehaves
contract Aggregator_USDM_SOL_megaeth_Test is WiredDoubleFeedHarness {
    ...
    function _priceFeeds() internal pure override returns (address[] memory feeds) {
        ...
        feeds[1] = SOL_USD.FEED;
    }

    function _createAggregator() internal override returns (address) {
        return address(new Aggregator_USDM_SOL_megaeth());
    }
}
